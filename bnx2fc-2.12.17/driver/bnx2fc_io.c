 ...
 *
 * Copyright (c) 2008-2018 Cavium, Inc.
 *
...
static void bnx2fc_scsi_done(struct bnx2fc_cmd *io_req, int err_code)
{
	...
	if (!sc_cmd->device) {
		printk(KERN_ERR PFX "0x%x: sc_cmd->device is NULL.\n",
		    io_req->xid);
		return;
	}
	if (!sc_cmd->device->host) {
		printk(KERN_ERR PFX "0x%x: sc_cmd->device->host is NULL.\n",
		    io_req->xid);
		return;
	}
...
static int bnx2fc_initiate_tmf(struct scsi_cmnd *sc_cmd, u8 tm_flags)
{
	...
	if (tm_flags == FCP_TMF_TGT_RESET)
		interface->hba->stats.target_resets++;
	else if (tm_flags == FCP_TMF_LUN_RESET)
		interface->hba->stats.lun_resets++;
...
int bnx2fc_eh_abort(struct scsi_cmnd *sc_cmd)
{
	...
	/* Sanity check to make sure we've released all references */
#ifdef _KREF_READ_
	WARN(kref_read(&io_req->refcount) > 1,
#else
	WARN(atomic_read(&io_req->refcount.refcount) > 1,
#endif
	    "xid=0x%x refcount=%d", io_req->xid,
#ifdef _KREF_READ_
	    kref_read(&io_req->refcount));
#else
	    atomic_read(&io_req->refcount.refcount));
#endif
	return rc;
}
				  ...
				  u8 num_rq)
{
	...
	if (io_req->wait_for_cleanup_comp) {
		BNX2FC_IO_DBG(io_req, "cleanup_compl: wakeup cleanup_done\n");
...
static int bnx2fc_map_sg(struct bnx2fc_cmd *io_req)
{
	...
	BUG_ON(scsi_sg_count(sc) > BNX2FC_MAX_BDS_PER_CMD);
	/*
...
static int bnx2fc_build_bd_list_from_sg(struct bnx2fc_cmd *io_req)
{
	...
	if (bd_count > BNX2FC_FW_MAX_BDS_PER_CMD) {
		printk(KERN_ERR PFX "bd_count = %d increased FW supported "
			"max BD(255), task_id = 0x%x\n",
			bd_count, io_req->xid);
		return -ENOMEM;
	}
				   ...
				   u8 num_rq, unsigned char *rq_data)
{
	...
	if (test_and_set_bit(BNX2FC_FLAG_IO_COMPL, &io_req->req_flags)) {
		...
		BNX2FC_IO_DBG(io_req, "Timer context finished processing "
			   "this scsi cmd\n");
		if (test_and_clear_bit(BNX2FC_FLAG_IO_CLEANUP,&io_req->req_flags)) {
			BNX2FC_IO_DBG(io_req, "Actual completion after cleanup request "
				"cleaning up\n");
