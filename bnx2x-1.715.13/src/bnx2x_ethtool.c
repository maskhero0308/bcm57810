...
/* 40 */{ STATS_OFFSET32(tx_stat_etherstatspkts512octetsto1023octets_hi),
	...
	{ STATS_OFFSET32(ptp_skip_txts),
			4, false, "Tx timestamps skipped"}
};
...
#if defined(_HAS_ETHTOOL_GET_LINK_KSETTINGS) && !RHEL_IS_VERSION(7, 3) /* BNX2X_UPSTREAM */
				    ...
				    struct ethtool_link_ksettings *cmd)
{
	...
	DP(NETIF_MSG_TIMER, "ethtool_cmd: cmd %d\n"
	   "  supported 0x%x  advertising 0x%x  speed %u\n"
	   "  duplex %d  port %d  phy_address %d\n"
	   "  autoneg %d\n",
	   cmd->base.cmd, supported, advertising,
...
#if defined(__VMKLNX__) /* ! BNX2X_UPSTREAM */
		/* if BW setting was attempted when link was down, overwrite
		 * ethtool saved speed by the new speed requested using IOCTL
		*/
		if (bp->pending_max) {
				/* Convert the % to actual speed(Mbps) */
				speed = bp->pending_max * 100;
				ethtool_cmd_speed_set(cmd, speed);
				BNX2X_DEV_INFO("Overwrite the Speed to %d as "
								"BW was modified when link was Down\n", speed);
				bp->pending_max = 0;
		}
#endif
...
/* Return required bytes for GrcDump */
static int bnx2x_get_grcdump_len(struct bnx2x *bp)
{
	int dwords = 0;

	dwords += BYTES_TO_DWORDS(sizeof(struct dump_header));
	dwords += __bnx2x_get_regs_len(bp);

	return DWORDS_TO_BYTES(dwords);
}
...
_UP_STATIC int bnx2x_get_regs_len(struct net_device *dev)
{
	...
	/* GrcDump */
	regdump_len += REGDUMP_HEADER_SIZE_BYTES;
	regdump_len += bnx2x_get_grcdump_len(bp);

	/* Internal trace dump */
	regdump_len += REGDUMP_HEADER_SIZE_BYTES;
	regdump_len += bnx2x_dbg_internal_trace_get_dump_size(bp);
...
void bnx2x_get_regs_buff(struct bnx2x *bp, u32 *p)
{
	 ...
	 * to print the GRC timeouts using this macros, we use the same.
	 */
...
/* Dump all driver debug data into buffer */
void bnx2x_dump_data(struct bnx2x *bp, u8 *p)
{
	u32 offset = 0, feature_size;

	/* GrcDump: Begin */
	feature_size = bnx2x_get_grcdump_len(bp);

	*(u32 *)(p + offset) = bnx2x_calc_regdump_header(bp, GRC_DUMP,
			feature_size, 0);
	offset += REGDUMP_HEADER_SIZE_BYTES;

	bnx2x_get_regs_buff(bp, (u32 *)(p + offset));
	offset += feature_size;
	/* GrcDump: End */

	/* Internal Trace Dump: Begin */
	bnx2x_dbg_internal_trace_result(bp,
					p + offset + REGDUMP_HEADER_SIZE_BYTES,
					&feature_size);
	*(u32 *)(p + offset) = bnx2x_calc_regdump_header(bp, INTERNAL_TRACE,
			feature_size, 0);
	/* Internal Trace Dump: End */
}
...
{
	u8 fw_ver_len = sizeof(info->fw_version);
	char *fw_ver_ptr = info->fw_version;
	char mbi_ver[ETHTOOL_FWVERS_LEN];
	char mfw_ver[ETHTOOL_FWVERS_LEN];
	...
	snprintf(storm, ETHTOOL_FWVERS_LEN, " storm %d.%d.%d.%d",
		 BCM_5710_FW_MAJOR_VERSION, BCM_5710_FW_MINOR_VERSION,
		 BCM_5710_FW_REVISION_VERSION, BCM_5710_FW_ENGINEERING_VERSION);
	strlcat(info->version, storm, sizeof(info->version));

	bnx2x_fill_fw_str(bp, mfw_ver, ETHTOOL_FWVERS_LEN);

	shmem2_has = SHMEM2_HAS(bp, extended_dev_info_shared_addr);
	if (shmem2_has) {
		...
		mbi = REG_RD(bp, ext_dev_info_offset +
			     offsetof(struct extended_dev_info_shared_cfg,
				      mbi_version));
		if (mbi) {
			snprintf(mbi_ver, ETHTOOL_FWVERS_LEN, "mbi %d.%d.%d ",
				 (mbi & 0xff000000) >> 24,
				 (mbi & 0x00ff0000) >> 16,
				 (mbi & 0x0000ff00) >> 8);
			snprintf(fw_ver_ptr, fw_ver_len, "%s", mbi_ver);
			fw_ver_ptr += strlen(mbi_ver);
			fw_ver_len -= strlen(mbi_ver);
		}
	}

	snprintf(fw_ver_ptr, fw_ver_len, "%s", mfw_ver);

	strlcpy(info->bus_info, pci_name(bp->pdev), sizeof(info->bus_info));

#ifndef BNX2X_UPSTREAM /* ! BNX2X_UPSTREAM */
	/* The following tidbits are provided in upstream using other
	 * ethtool ops. No problem in having them here as well, though.
	 * [Other than for alignment purpose]
	 */
...
#ifdef __VMKLNX__  /* ! BNX2X_UPSTREAM */
	if (IS_MF_BD(bp) && SHMEM2_HAS(bp, curr_cfg)) {
		bnx2x_fw_command(bp, DRV_MSG_CODE_CONFIG_CHANGE,
...
static u32 bnx2x_get_msglevel(struct net_device *dev)
{
	...
	/* Check if internal msglevel was requested */
	if (!bp->internal_trace.is_int_msglevel)
		return bp->msg_enable;
	else
		return bp->internal_trace.level;
}
...
static u32 bnx2x_get_link(struct net_device *dev)
{
	...
	if ((bp->flags & MF_FUNC_DIS) || !BNX2X_STATE_OPEN_OR_EXPLICIT_LB(bp))
		return 0;
...
#define BNX2X_EEPROM_CHUNK_SIZE 128
#define BNX2X_REGADDR_MAGIC 0xcafecafe

static int bnx2x_get_eeprom(struct net_device *dev,
			    struct ethtool_eeprom *eeprom, u8 *eebuf)
{
	struct bnx2x *bp = netdev_priv(dev);
	int rc, offset, len, remaining;
	...
	if (eeprom->magic == BNX2X_REGADDR_MAGIC) {
		u32 val;

		val = REG_RD(bp, eeprom->offset << 2);
		memcpy(eebuf, (void *)&val, sizeof(u32));
		DP(BNX2X_MSG_ETHTOOL, "Value = 0x%x\n", val);
	...
	while (remaining) {
		len = remaining < BNX2X_EEPROM_CHUNK_SIZE ? remaining :
					BNX2X_EEPROM_CHUNK_SIZE;
		rc = bnx2x_nvram_read(bp, eeprom->offset + offset,
				      eebuf + offset, len);
		if (rc)
			return rc;
			    ...
			    struct ethtool_eeprom *eeprom, u8 *eebuf)
{
	...
	if (eeprom->magic == BNX2X_REGADDR_MAGIC) {
		...
		memcpy((void *)&val, eebuf, sizeof(u32));
		DP(BNX2X_MSG_ETHTOOL, "Value = 0x%x\n", val);
		REG_WR(bp, eeprom->offset << 2, val);
...
enum {
	...
	BNX2X_PRI_FLAG_PHY_LB,
	BNX2X_PRI_FLAG_MAC_LB,
	BNX2X_PRI_FLAG_EXT_LB,
#endif
	BNX2X_PRI_FLAG_SET_INT_MSGLVL, /* Indicate msglevel is for int trace */
	BNX2X_PRI_FLAG_LEN,
};
...
static int bnx2x_test_nvram(struct bnx2x *bp)
{
	...
	magic = (u32 *)cbuf;
	rc = bnx2x_nvram_read32(bp, 0, magic, sizeof(cbuf));
	if (rc) {
		DP(BNX2X_MSG_ETHTOOL | BNX2X_MSG_NVM,
...
void bnx2x_self_test(struct net_device *dev,
...
#ifdef BNX2X_ALLOW_LB /* ! BNX2X_UPSTREAM */
	flags |= ((bp->lb_mode == LB_LOAD_MODE_PHY) ? 1 : 0) <<
		 BNX2X_PRI_FLAG_PHY_LB;
	flags |= ((bp->lb_mode == LB_LOAD_MODE_MAC) ? 1 : 0) <<
		 BNX2X_PRI_FLAG_MAC_LB;
	flags |= ((bp->lb_mode == LB_LOAD_MODE_EXT) ? 1 : 0) <<
		 BNX2X_PRI_FLAG_EXT_LB;
#endif
	flags |= (bp->internal_trace.is_int_msglevel ? 1 : 0)
		 << BNX2X_PRI_FLAG_SET_INT_MSGLVL;

	return flags;
}

static int bnx2x_set_private_flags(struct net_device *dev, u32 flags)
{
	struct bnx2x *bp = netdev_priv(dev);

#ifdef BNX2X_ALLOW_LB /* ! BNX2X_UPSTREAM */
	u8 need_mac, need_phy, need_ext;

	need_mac = !!(flags & (1 << BNX2X_PRI_FLAG_MAC_LB));
	need_phy = !!(flags & (1 << BNX2X_PRI_FLAG_PHY_LB));
	need_ext = !!(flags & (1 << BNX2X_PRI_FLAG_EXT_LB));

	if (need_mac + need_phy + need_ext > 1) {
		DP(BNX2X_MSG_ETHTOOL,
		   "Can only support one LB mode at a time\n");
		return -EINVAL;
	}

	if (need_mac)
		bp->lb_mode = LB_LOAD_MODE_MAC;
	else if (need_phy)
		bp->lb_mode = LB_LOAD_MODE_PHY;
	else if (need_ext)
		bp->lb_mode = LB_LOAD_MODE_EXT;
	else
		bp->lb_mode = LB_LOAD_MODE_NONE;
	...
	bp->internal_trace.is_int_msglevel =
		!!(flags & (1 << BNX2X_PRI_FLAG_SET_INT_MSGLVL));
...
#if defined(_HAS_NEW_ETHTOOL_OPS) /* ! BNX2X_UPSTREAM */
static const struct new_ethtool_ops bnx2x_ethtool_ops = {
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 19)) /* BNX2X_UPSTREAM */
static const struct ethtool_ops bnx2x_ethtool_ops = {
#else
static struct ethtool_ops bnx2x_ethtool_ops = {
#endif
#if defined(_HAS_ETHTOOL_SUPPORTED_COALESCE_PARAMS) /* BNX2X_UPSTREAM */
...
#endif
#if (defined(_HAS_NEW_ETHTOOL_OPS)) /* ! BNX2X_UPSTREAM */
	.get_settings = (void *)(-EPERM),
	.set_settings = (void *)(-EPERM),
#endif
	...
	.set_priv_flags		= bnx2x_set_private_flags,
#else
...
#if (defined(_HAS_ETHTOOL_OPS_EXT) && RHEL_PRE_VERSION(8, 0)) /* ! BNX2X_UPSTREAM */
...
#if defined(_HAS_NEW_ETHTOOL_OPS) /* ! BNX2X_UPSTREAM */
static const struct new_ethtool_ops bnx2x_vf_ethtool_ops = {
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 19)) /* BNX2X_UPSTREAM */
...
void bnx2x_set_ethtool_ops(struct bnx2x *bp, struct net_device *netdev)
{
	...
	if (IS_PF(bp))
		netdev->ethtool_ops = (const struct ethtool_ops *)
					&bnx2x_ethtool_ops;
	else
		netdev->ethtool_ops = (const struct ethtool_ops *)
