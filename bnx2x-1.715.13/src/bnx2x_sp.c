				       ...
				       struct bnx2x_pending_mcast_cmd *cmd_pos)
{
	...
	list_for_each_entry_safe(pmac_pos, pmac_pos_n, &cmd_pos->data.macs_head,
		...
		DP(BNX2X_MSG_SP, "Set contains " BNX2X_MAC_FMT " mcast MAC\n",
	...
	if (CHIP_IS_E1x(bp)) {
		...
		if (func_num > 0) {
			...
			if (IS_VLAN_FILTER(bp))
				credit = BNX2X_PF_VLAN_CREDIT_VLAN_FILTERING;
			else
				credit = PF_VLAN_CREDIT_E2(bp, func_num);
...
#if defined(_NTDDK_)
#pragma warning (pop)
#endif /* (_NTDDK_) 26110, 28182, 6011 */
				...
				unsigned long *flags)
{
	...
	rx_data->tpa_en |= test_bit(BNX2X_Q_FLG_TPA_VLAN_DIS, flags) *
				...
				struct tpa_update_ramrod_data *data)
{
	...
	data->tpa_over_vlan_disable = params->disable_tpa_over_vlan;
}
					...
					struct bnx2x_func_state_params *params)
{
	...
	rdata->tx_vlan_filtering_use_pvid =
				start_params->tx_vlan_filtering_use_pvid;
	rdata->tx_vlan_filtering_enable =
				start_params->tx_vlan_filtering_enable;
					...
					struct bnx2x_func_state_params *params)
{
	...
	if (test_bit(BNX2X_F_UPDATE_VLAN_FORCE_PRIO_CHNG,
		...
		rdata->sd_vlan_force_pri_val =
			switch_update_params->vlan_force_prio;
	}

	if (test_bit(BNX2X_F_UPDATE_TUNNEL_INNER_CLSS_VXLAN_INNER_VNI,
		     &switch_update_params->changes))
		inner_vni = 1;
	...
	if (test_bit(BNX2X_F_UPDATE_TUNNEL_CFG_CHNG,
		...
		if (test_bit(BNX2X_F_UPDATE_TUNNEL_INNER_CLSS_VXLAN,
			...
			if (inner_vni == 1)
				rdata->inner_clss_vxlan = INNER_CLSS_USE_VNI;
		}
	...
	if (test_bit(BNX2X_F_UPDATE_VLAN_FILTERING_PVID_CHNG,
		     &switch_update_params->changes)) {
		rdata->tx_vlan_filtering_pvid_change_flg = 1;
		rdata->sd_vlan_tag_change_flg = 1;
		rdata->sd_vlan_tag = cpu_to_le16(switch_update_params->vlan);
	}
