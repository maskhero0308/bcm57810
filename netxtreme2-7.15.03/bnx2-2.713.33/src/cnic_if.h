...
#define CNIC_MODULE_VERSION	"2.5.21g"
#define CNIC_MODULE_RELDATE	"OCT 7, 2021"
...
/* For iscsiuio rings use L2 driver page size */
#define CNIC_PG_SZ_L2	BCM_PAGE_SIZE

#define CNIC_PATH_GARP_HANDLE 0xFE000001
...
struct link_error_status {
	...
	u32 loss_sync_cnt;
	u32 loss_sig_cnt;
	u32 prim_seq_err_cnt;
	u32 inval_xmit_word_cnt;
	u32 inval_crc_cnt;
};
...
#define CNIC_DRV_STATE_HANDLES_IRQ	0x00000020
	...
	int		(*drv_get_fc_npiv_tbl)(struct net_device *,
					       struct cnic_fc_npiv_tbl *);
	int		(*drv_get_sfp_diagnostic)(struct net_device *,
						  struct sfp_diagnostic *);
	int		(*drv_get_link_error)(struct net_device *,
					      struct link_error_status *);
...
#define CNIC_DEV_VER 0xcdef0006 /* Change this when the structure changes */
	...
	int (*drv_get_sfp_diagnostic)(struct cnic_dev *,
				      struct sfp_diagnostic *);
	int (*drv_get_link_error)(struct cnic_dev *, struct link_error_status *);
