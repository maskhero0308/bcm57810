...
#ifndef __VMKLNX__ /* BNX2X_UPSTREAM */
	/* Since we don't know anything about the VF, always prevent this
	 * for the VF's sake.
	 */
	__set_bit(BNX2X_Q_FLG_TPA_VLAN_DIS, &setup_p->flags);
#endif
...
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(4, 16, 0)) /* ! BNX2X_UPSTREAM */
...
static void bnx2x_vf_flr_clnup_hw(struct bnx2x *bp, struct bnx2x_virtf *vf)
{
	...
	/* DQ usage counter */
	bnx2x_pretend_func(bp, HW_VF_HANDLE(bp, vf->abs_vfid));
	if (bnx2x_flr_clnup_poll_hw_counter(bp, DORQ_REG_VF_USAGE_CNT,
					    "DQ VF usage counter timed out",
					    poll_cnt))
		BNX2X_ERR("VF[%d] Flr cleanup failed\n", vf->abs_vfid);
	bnx2x_pretend_func(bp, BP_ABS_FUNC(bp));
		       ...
		       int num_vfs_param)
{
	...
	for_each_vf(bp, i) {
		...
		/* keep spoofchk enabled by default */
		bnx2x_vf(bp, i, spoofchk) = true;

#ifndef BNX2X_UPSTREAM /* ! BNX2X_UPSTREAM */
#ifndef __VMKLNX__ /* BNX2X_UPSTREAM */
		if (use_random_vf_mac)
			eth_random_addr(bnx2x_vf(bp, i, default_mac));
#endif
#endif
	}
...
void bnx2x_iov_adjust_stats_req(struct bnx2x *bp)
{
	...
	for_each_vf(bp, i) {
		...
		for_each_vfq(vf, j) {
			...
			size_t size = (size_t)vf->stats_stride * (size_t)j;

			dma_addr_t q_stats_addr = vf->fw_stat_map + size;
...
#ifndef BNX2X_UPSTREAM /* ! BNX2X_UPSTREAM */
	/* If module parameter is given, prevent dynamic changes */
	if (tx_switching)
		return 0;
#endif
...
#ifndef BNX2X_UPSTREAM /* BNX2X_UPSTREAM */
#ifndef __VMKLNX__ /* BNX2X_UPSTREAM */
		/* Force VF-macs, if appropriate. This wouldn't take affect
		 * now, but would be cached until VF is acquired.
		 */
		if (use_random_vf_mac)
			bnx2x_set_vf_mac(bp->dev, vf_idx, vf->default_mac);
#endif
#endif
	}
...
#if HAS_NDO(VF_VLAN_PROTO) /* BNX2X_UPSTREAM */
...
#ifdef _HAS_BRIDGE_GETLINK_V3/* BNX2X_UPSTREAM */
int bnx2x_bridge_getlink(struct sk_buff *skb, u32 pid, u32 seq,
			 struct net_device *dev,
			 u32 filter_mask, int nlflags)
#elif defined(_HAS_BRIDGE_GETLINK_V2)
int bnx2x_bridge_getlink(struct sk_buff *skb, u32 pid, u32 seq,
			 struct net_device *dev, u32 filter_mask)
#else
int bnx2x_bridge_getlink(struct sk_buff *skb, u32 pid, u32 seq,
			 struct net_device *dev)
#endif /*_HAS_BRIDGE_GETLINK_V3 */
{
	struct bnx2x *bp = netdev_priv(dev);
	u16 mode;
...
#ifdef _HAS_DFLT_BRIDGE_GETLINK_V4 /* BNX2X_UPSTREAM */
	return ndo_dflt_bridge_getlink(skb, pid, seq, dev, mode, 0,
				       0, nlflags, filter_mask, NULL);
#elif defined(_HAS_DFLT_BRIDGE_GETLINK_V3)
	return ndo_dflt_bridge_getlink(skb, pid, seq, dev, mode, 0, 0, nlflags);
#elif defined(_HAS_DFLT_BRIDGE_GETLINK_V2)
...
#else
	struct bnx2x *bp = container_of(work, struct bnx2x, iov_task);
#endif
#else
...
#else
	queue_work(bnx2x_iov_wq, &bp->iov_task);
#endif
}
