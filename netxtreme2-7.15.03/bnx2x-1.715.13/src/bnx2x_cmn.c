...
#endif
#if !defined(_HAS_NAPI_HASH_AUTO) && !defined(_DEFINE_SKB_MARK_NAPI_ID) /* ! BNX2X_UPSTREAM */
...
#ifndef _HAS_NAPI_HASH_AUTO /* ! BNX2X_UPSTREAM */
...
#else
	mapping = pci_map_page(bp->pdev, pool->page,
			       pool->offset, SGE_PAGE_SIZE, PCI_DMA_FROMDEVICE);
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)) /* BNX2X_UPSTREAM */
	if (unlikely(dma_mapping_error(&bp->pdev->dev, mapping))) {
#else
...
reuse_rx:
		...
		if ((bp->vlgrp != NULL) &&
		    (le16_to_cpu(cqe_fp->pars_flags.flags) &
		     PARSING_FLAGS_VLAN)) {
#else
		if (((!IS_FCOE_FP(fp) && bp->vlgrp != NULL) ||
		     (IS_FCOE_FP(fp) && bp->cna_vlgrp != NULL)) &&
		    (le16_to_cpu(cqe_fp->pars_flags.flags) &
		     PARSING_FLAGS_VLAN)) {
...
#if defined(__VMKLNX__) /* !BNX2X_UPSTREAM */
void bnx2x_free_tx_skbs(struct bnx2x *bp)
#else
...
#ifdef BNX2X_MULTI_QUEUE /* BNX2X_UPSTREAM */
#ifdef BNX2X_SELECTQUEUE_HAS_SBDEV_PARAM /* BNX2X_UPSTREAM */
u16 bnx2x_select_queue(struct net_device *dev, struct sk_buff *skb,
		       struct net_device *sb_dev)
#elif defined(BNX2X_SELECTQUEUE_HAS_FALLBACK_SBDEV_PARAM)
u16 bnx2x_select_queue(struct net_device *dev, struct sk_buff *skb,
		       struct net_device *sb_dev,
		       select_queue_fallback_t fallback)
#elif defined(BNX2X_SELECTQUEUE_HAS_FALLBACK_PARAM)
u16 bnx2x_select_queue(struct net_device *dev, struct sk_buff *skb,
...
int bnx2x_nic_load(struct bnx2x *bp, int load_mode)
{
   ...
   if(bp->rss_on_default_queue){
	...
	if (bp->lb_mode != LB_LOAD_MODE_NONE)
		bp->state = BNX2X_STATE_DIAG;
#endif
...
int bnx2x_nic_unload(struct bnx2x *bp, int unload_mode, bool keep_link)
{
	 ...
	 * to wait for the queue to complete all Tx.
	 */
...
#else
	rx_work_done = (bnx2x_has_rx_work(fp)) ?
		       bnx2x_rx_int(fp, min(*budget, dev->quota)) : 0;
	*budget -= rx_work_done;
	dev->quota -= rx_work_done;

	if (min(*budget, dev->quota)) {
#endif
...
#ifndef BNX2X_UPSTREAM /* ! BNX2X_UPSTREAM */
	/* There was quite a bit of versioning mahyam here;
	 * I've aligned everything to upstream, but notice this
	 * might break functionality on very old kernels;
	 * The reason I'm allowing to this change is due to the fact
	 * the oldest distro I've had at this time [RH5.5 source code]
	 * looks like it would work fine with this.
	 * Still, if Traffic broken on very old distro [2.6.18] this might
	 * be the reason.
	 */
#endif
...
#if defined(_HAS_TC_SETUP_TYPE) || defined(_HAS_NDO_EXT_SETUP_TC) /* BNX2X_UPSTREAM */
...
#if defined(__VMKLNX__) /* ! BNX2X_UPSTREAM */
	if (IS_MF_BD(bp) && !rc &&
	    IS_PF(bp) && SHMEM2_HAS(bp, curr_cfg)) {
...
#if !defined(_HAS_MAX_MTU) && !defined(_HAS_EXT_MAX_MTU) /* ! BNX2X_UPSTREAM */
...
#ifdef _HAS_NDO_TX_TIMEOUT_TXQUEQUE
...
#endif
{
	...
	if (bp->recovery_state != BNX2X_RECOVERY_DONE) {
		 ...
		 */
#if defined(__VMKLNX__) /* ! BNX2X_UPSTREAM */
		BNX2X_ERR("ESX_REC_BSY DS:0x%x RS:%d ES:0x%x\n",
...
#else
		netdev_info(dev,
			    "tx timeout while recovery already in progress\n");
#endif
...
#if !defined(__VMKLNX__) /* BNX2X_UPSTREAM */
	netdev_info(dev, "bnx2x_tx_timeout !!\n");
...
void bnx2x_schedule_sp_rtnl_delay(struct bnx2x *bp, enum sp_rtnl_flag flag,
				  u32 verbose, unsigned long delay)
{
	smp_mb__before_atomic();
	set_bit(flag, &bp->sp_rtnl_state);
	smp_mb__after_atomic();
	DP((BNX2X_MSG_SP | verbose), "Scheduling sp_rtnl task [Flag: %d]\n", flag);
	schedule_delayed_work(&bp->sp_rtnl_task, delay);
}

void bnx2x_schedule_sp_rtnl(struct bnx2x *bp, enum sp_rtnl_flag flag,
			    u32 verbose)
{
	bnx2x_schedule_sp_rtnl_delay(bp, flag, verbose, 0);
}
