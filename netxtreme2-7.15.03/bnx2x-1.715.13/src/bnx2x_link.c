...
#define CL22_RD_OVER_CL45(_bp, _phy, _bank, _addr, _val) \
	bnx2x_cl45_read(_bp, _phy, \
		...
		(_bank + (_addr & 0xf)), \
		_val)
int bnx2x_debug_data_bp(struct link_params *params,
			u32 *dbg_data_hdr,
			u8 **dbg_data_ptr)
{
	/* Form the debug data header without data and block size */
	*dbg_data_hdr = 0;

	switch (params->dbg_data_type) {
	case DBG_DATA_TYPE_ELINK_PHY_INIT:
		*dbg_data_hdr |= DBG_DATA_VER_ELINK_PHY_INIT;
		break;
	case DBG_DATA_TYPE_LINK_UPDATE:
		*dbg_data_hdr |= DBG_DATA_VER_LINK_UPDATE;
		break;
	case DBG_DATA_TYPE_PERIODIC_POLL:
		*dbg_data_hdr |= DBG_DATA_VER_PERIODIC_POLL;
		/* send the periodic data if there is any event change */
		if (params->last_periodic_status ==
			params->dbg_data_buffer[0]) {
			params->dbg_data_size = 0;
		} else {
			params->last_periodic_status = params->dbg_data_buffer[0];
		}
		break;
	case DBG_DATA_TYPE_LINK_RESET:
		*dbg_data_hdr |= DBG_DATA_VER_LINK_RESET;
		break;
	default:
		break;
	}
	*dbg_data_ptr = (u8 *)&params->dbg_data_buffer[0];
	return params->dbg_data_size;
}

#define ADD_DBG_DATA(params, reg, data) \
	(params->dbg_data_buffer[DBG_ ## reg ## _IDX] = data)

#define ADD_DBG_DATA_BITS(params, reg, data) \
	(params->dbg_data_buffer[DBG_ ## reg ## _IDX] = \
	(params->dbg_data_buffer[DBG_ ## reg ## _IDX] & \
	(~(DBG_ ## reg ## _MASK << DBG_ ## reg ## _OFFSET))) | \
	(data << DBG_ ## reg ## _OFFSET))

#define ADD_DBG_DATA_SIZE(size) (params->dbg_data_size = (size))
#define ADD_DBG_DATA_TYPE(type) (params->dbg_data_type = type)

static void bnx2x_memset(void *buf, int len, u8 val)
{
	u8 *buffer = (u8 *)buf;
	int i;
		...
		struct bnx2x_nig_brb_pfc_port_params *nig_params)
{
	...
	/* Disable BRB->NIG signal to indicate pause deassertion
	 * to XMAC before disabling Tx HW PFC.
	 */
	REG_WR(bp, port ? NIG_REG_LLFC_EGRESS_SRC_ENABLE_1 :
	       NIG_REG_LLFC_EGRESS_SRC_ENABLE_0, 0x0);
				       ...
				       u16 reg, u16 val)
{
	...
	/* Address */
	tmp = (((u32)phy->addr << 21) | ((u32)reg << 16) | val |
				      ...
				      u16 reg, u16 *ret_val)
{
	...
	/* Address */
	val = (((u32)phy->addr << 21) | ((u32)reg << 16) |
			      ...
			      u32 *data_array)
{
	...
	/* Add delay to meet SFF-8431 tBUF sepcification in case of
	 *  back to back EEPROM read.
	 */
	if (sl_devid == I2C_DEV_ADDR_A0 ||
	    sl_devid == I2C_DEV_ADDR_A2)
		udelay(30);
	...
	/* Add delay to meet SFF-8431 tBUF sepcification. */
	if (sl_devid == I2C_DEV_ADDR_A0 ||
	    sl_devid == I2C_DEV_ADDR_A2)
		udelay(30);
...
{
	struct bnx2x *bp = params->bp;
	struct bnx2x_phy *extphy = &params->phy[EXT_PHY1];
	...
	bnx2x_warpcore_set_lpi_passthrough(phy, params);
	if (always_autoneg || speed == SPEED_AUTO_NEG) {
		/* SGMII Autoneg */
		bnx2x_cl45_read_or_write(bp, phy, MDIO_WC_DEVAD,
	...
	/* Disable filter force link */
	bnx2x_cl45_read(bp, phy, MDIO_WC_DEVAD,
			MDIO_WC_REG_SERDESDIGITAL_CONTROL1000X2, &digctrl_kx2);
	digctrl_kx2 &= ~FILTER_FORCE_LINK;
	bnx2x_cl45_write(bp, phy, MDIO_WC_DEVAD,
			MDIO_WC_REG_SERDESDIGITAL_CONTROL1000X2,
			digctrl_kx2);
	digctrl_kx2 |= FILTER_FORCE_LINK;
	if (extphy->type == PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BCM84858)
		digctrl_kx2 &= ~EN_PARALLEL_DET;
	else
		digctrl_kx1 |= AUTODET_EN;
...
int bnx2x_warpcore_get_sigdet(struct bnx2x_phy *phy,
			      struct link_params *params)
{
	...
	bnx2x_cl45_read(bp, phy, MDIO_WC_DEVAD, MDIO_WC_REG_GP2_STATUS_GP_2_0,
				 &gp2_status_reg0);
	ADD_DBG_DATA_BITS(params, WC_REG_GP2_2_0, gp2_status_reg0);
				       ...
				       struct link_vars *vars)
{
	...
	if ((serdes_net_if == PORT_HW_CFG_NET_SERDES_IF_SGMII) ||
		...
		bnx2x_warpcore_set_sgmii_speed(phy, params, 0, (u8)always_autoneg,
					       phy->req_line_speed);
	} else {
		switch (serdes_net_if) {
		case PORT_HW_CFG_NET_SERDES_IF_KR:
		...
		case PORT_HW_CFG_NET_SERDES_IF_XFI:
			...
			if (vars->line_speed == SPEED_10000) {
				...
				if (params->link_flags &
				    WC_SGMII_FORCE_SPEED)
					speed = vars->line_speed;
				else
					speed = phy->req_line_speed;
				     ...
				     struct link_vars *vars)
{
	...
	if ((params->loopback_mode) &&
		...
		link_up = ((gp_status1 >> 8) |
			   ...
			   (gp_status1)) &
			(1 << lane);
		ADD_DBG_DATA_BITS(params, MDIO_WC_REG_GP2_STAT_GP_2_1,
				  gp_status1);
		...
		if (link_up && SINGLE_MEDIA_DIRECT(params)) {
			...
			if (phy->req_line_speed == SPEED_AUTO_NEG) {
				...
				if (gp_status4 & ((1<<12)<<lane))
					vars->link_status |=
					LINK_STATUS_AUTO_NEGOTIATE_COMPLETE;
				ADD_DBG_DATA_BITS(params,
						  MDIO_WC_REG_GP2_STAT_GP_2_4,
						  gp_status4);
	...
	if ((vars->link_status & LINK_STATUS_AUTO_NEGOTIATE_COMPLETE) &&
		...
		ADD_DBG_DATA_BITS(params, MDIO_AN_REG_LP_AUTO_NEG2, val);
		...
		ADD_DBG_DATA_BITS(params, MDIO_WC_REG_DIGITAL3_LP_UP1, val);
	...
	ADD_DBG_DATA_BITS(params, MDIO_WC_REG_GP2_STAT_GP_2_23, gp_speed);
...
/* Format the version information to string when there are 3 sequences
 * in the version, MAJ.MIN.BUILD.
 * MAJ = bits[23:16] , MIN = bits[15:8] , BUILD = bits [7:0]
 */
		  ...
		  struct link_vars *vars, u8 mode, u32 speed)
{
	...
	case LED_MODE_OPER:
		...
		/* fall through */
	case LED_MODE_ON:
		/* BCM8722 and BCM8727 are used only in E1 E2 design */
		if (((params->phy[EXT_PHY1].type ==
				 ...
				 struct link_vars *vars)
{
	...
	if (non_ext_phy) {
		...
		for (phy_index = EXT_PHY1; phy_index < params->num_phys;
			...
			if (params->phy[phy_index].config_init) {
...
static void bnx2x_chng_link_count(struct link_params *params, u8 clear)
{
	struct bnx2x *bp = params->bp;
	u32 addr, val;
	...
	REG_WR(bp, addr, val);
	ADD_DBG_DATA(params, LNK_CHANGE_COUNT, val);
}
...
int bnx2x_link_update(struct link_params *params, struct link_vars *vars)
{
	...
	bnx2x_memset(params->dbg_data_buffer, MAX_DBG_DATA_DW_SIZE * 4, 0);
	temp1 = REG_RD(bp, NIG_REG_STATUS_INTERRUPT_PORT0 + port*4);
	ADD_DBG_DATA(params, NIG_REG_STAT_INTERRUPT_PORT0, temp1);

	DP(NETIF_MSG_LINK, "port %x, XGXS?%x, int_status 0x%x\n",
		 port, (vars->phy_flags & PHY_XGXS_FLAG), temp1);

	temp1 = REG_RD(bp, NIG_REG_EMAC0_STATUS_MISC_MI_INT + port*0x18);
	ADD_DBG_DATA_BITS(params, NIG_REG_EMAC0_STAT_MISC_MI_INT, temp1);

	temp2 =  REG_RD(bp, NIG_REG_SERDES0_STATUS_LINK_STATUS + port*0x3c);
	ADD_DBG_DATA_BITS(params, NIG_REG_SERDES0_STAT_LINK_STAT, temp2);

	DP(NETIF_MSG_LINK, "int_mask 0x%x MI_INT %x, SERDES_LINK %x\n",
		 REG_RD(bp, NIG_REG_MASK_INTERRUPT_PORT0 + port*4),
		 temp1, temp2);

	temp1 = REG_RD(bp, NIG_REG_XGXS0_STATUS_LINK10G + port*0x68);
	ADD_DBG_DATA_BITS(params, NIG_REG_XGXS0_STAT_LINK10G, temp1);

	temp2 = REG_RD(bp, NIG_REG_XGXS0_STATUS_LINK_STATUS + port*0x68);
	ADD_DBG_DATA_BITS(params, NIG_REG_XGXS0_STAT_LINK_STAT, temp2);
	...
	if ((prev_link_status ^ vars->link_status) & LINK_STATUS_LINK_UP)
		bnx2x_chng_link_count(params, 0);
	...
	ADD_DBG_DATA_SIZE(DBG_DATA_LINK_UPDATE_SIZE);
	ADD_DBG_DATA_TYPE(DBG_DATA_TYPE_LINK_UPDATE);
	return rc;
}
						 ...
						 u8 *o_buf, u8 is_init)
{
	...
	if (rc == 0) {
		/* Addition converts to signed int so need to cast to
		 * unsigned.
		 */
		for (i = (addr - addr32);
			i < (u32)(byte_cnt + (addr - addr32)); i++) {
...
#define FINISAR_UUID  0x009065
static int bnx2x_get_edc_mode(struct bnx2x_phy *phy,
			      struct link_params *params,
			      u16 *edc_mode)
{
	struct bnx2x *bp = params->bp;
	u32 sync_offset = 0, phy_idx, media_types, uuid;
	...
	*edc_mode = EDC_MODE_LIMITING;
	...
	if (bnx2x_read_sfp_module_eeprom(phy,
					 params,
					 I2C_DEV_ADDR_A0,
					 SFP_EEPROM_VENDOR_OUI_ADDR,
					 SFP_EEPROM_VENDOR_OUI_SIZE,
					 (u8 *)val) != 0) {
		DP(NETIF_MSG_LINK, "Failed to read from SFP+ module EEPROM\n");
		return -EINVAL;
	}
	uuid = (val[0] << 16) | (val[1] << 8) | val[2];
	...
	bnx2x_update_link_attr(params, params->link_attr_sync);
	/* Force the connector type to Optical in case of Finisar AOC cable */
	if ((uuid == FINISAR_UUID) && (val[SFP_EEPROM_FC_TX_TECH_ADDR] ==
		     SFP_EEPROM_FC_TX_TECH_BITMASK_COPPER_ACTIVE)) {
		val[SFP_EEPROM_CON_TYPE_ADDR] =
			SFP_EEPROM_CON_TYPE_VAL_LC;
		val[SFP_EEPROM_10G_COMP_CODE_ADDR] =
			       ...
			       struct link_params *params)
{
	...
	if (CHIP_IS_E3(bp)) {
		/* Enable the MAC to handle the Local/Remote fault in case
		 * there is a half open connector even before the link is up
		 */
		if (phy->req_line_speed == SPEED_10000)
			bnx2x_xmac_init(params, phy->req_line_speed);
	}
				      ...
				      u32 action)
{
	...
	case PHY_INIT:
		...
		break;
	case PHY_PORT_PWR_DOWN:
		if (phy->type == PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BCM84858) {
			u16 addr = MDIO_AN_REG_8481_LEGACY_MII_CTRL;
				       ...
				       struct link_vars *vars)
{
	...
	if (((phy->req_line_speed == SPEED_AUTO_NEG) &&
		...
		bnx2x_cl45_write(bp, phy,
				 ...
				 1);
	if (phy->type == PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BCM84858)
		params->link_flags = WC_SGMII_FORCE_SPEED;
	return 0;
}
...
#define PHY_CMD_TIMEOUT	2000
#define PHY848xx_CMDHDLR_WAIT 300
#define CMDHDLR_SLEEP	50
#define PHY848xx_CMDHDLR_WAIT_LOOP \
	(PHY848xx_CMDHDLR_WAIT / CMDHDLR_SLEEP)
				...
				u16 cmd_args[], int argc, int process)
{
	...
	if (process == PHY84833_MB_PROCESS2) {
		...
		if (bnx2x_cl45_write(bp, phy, MDIO_CTL_DEVAD,
				     MDIO_848xx_CMD_HDLR_STATUS,
				     PHY84833_STATUS_CMD_OPEN_OVERRIDE)) {
			return -EINVAL;
		};
	}
	for (idx = 0; idx < PHY848xx_CMDHDLR_WAIT_LOOP; idx++) {
		if (bnx2x_cl45_read(bp, phy, MDIO_CTL_DEVAD,
				    MDIO_848xx_CMD_HDLR_STATUS, &val)) {
			return -EINVAL;
		}
		/* For Process2 sequence wait till MB is ready.
		 * Otherwise proceed.
		 */
	...
	if (val != PHY84833_STATUS_CMD_OPEN_FOR_CMDS) {
		...
		/* Issue NOP command if we still see below status */
		if (val == PHY84833_STATUS_CMD_RECEIVED ||
		    val == PHY84833_STATUS_CMD_IN_PROGRESS) {
			bnx2x_cl45_write(bp, phy, MDIO_CTL_DEVAD,
					 MDIO_848xx_CMD_HDLR_COMMAND, 0);
		}
		return -EINVAL;
	}
	...
	if ((idx >= PHY848xx_CMDHDLR_WAIT_LOOP) ||
	    (val == PHY84833_STATUS_CMD_COMPLETE_ERROR)) {
		DP(NETIF_MSG_LINK, "FW cmd failed.\n");
		rc = -EINVAL;
	}
					   ...
					   int process)
{
	...
	if ((phy->type == PORT_HW_CFG_XGXS_EXT_PHY_TYPE_BCM84858) ||
		...
		/* Issue the command till successfull or timeout */
		for (i = 0; i < PHY_CMD_TIMEOUT / CMDHDLR_SLEEP; i++) {
			status = bnx2x_84833_cmd_hdlr(phy, params, fw_cmd,
						      cmd_args, argc, process);
			/* For temperature read do not wait for the
			 * command to complete
			 */
			if (status == 0 ||
			    fw_cmd == PHY848xx_CMD_GET_CURRENT_TEMP)
				break;
			msleep(50);
		}
		return status;
				   ...
				   struct link_vars *vars)
{
	...
	if (status == 0) {
		...
		DP(NETIF_MSG_LINK, "Pairswap Not OK\n");
	}
					       ...
					       struct link_vars *vars)
{
	...
	if ((phy->flags & FLAGS_EEE) && bnx2x_eee_has_cap(params)) {
		...
		vars->eee_status &= (u32)(((u64)~SHMEM_EEE_1G_ADV) <<
	...
	bnx2x_cl22_write(bp, phy,
			MDIO_REG_GPHY_SHADOW_ACCESS,
			(MDIO_REG_GPHY_SHADOW_AUX_CTRL << 12) |
			MDIO_REG_GPHY_SHADOW_MISC_CTRL);

	bnx2x_cl22_read(bp, phy, MDIO_REG_GPHY_SHADOW_ACCESS, &temp);
	temp &= 0xfff8;

	bnx2x_cl22_write(bp, phy,
			MDIO_REG_GPHY_SHADOW_ACCESS,
			temp | (1 << 14) | MDIO_REG_GPHY_SHADOW_AUX_CTRL);

	bnx2x_cl22_read(bp, phy, MDIO_REG_GPHY_BASET_EXT_CTRL, &temp);

	bnx2x_cl22_write(bp, phy,
			MDIO_REG_GPHY_BASET_EXT_CTRL,
			(temp | MDIO_REG_GPHY_TX_HIGH_LATENCY));
...
int bnx2x_phy_init(struct link_params *params, struct link_vars *vars)
{
	...
	ADD_DBG_DATA(params, REQ_SPEED, params->req_line_speed[0]);
	ADD_DBG_DATA_BITS(params, REQ_FLOW_CTRL, params->req_flow_ctrl[0]);
	ADD_DBG_DATA(params, EEE_MODE, params->eee_mode);
	ADD_DBG_DATA_BITS(params, LOOPBACK_MODE, params->loopback_mode);
	ADD_DBG_DATA(params, FEAT_CFG_FLAGS, params->feature_config_flags);
	ADD_DBG_DATA(params, SPEED_CAP, params->speed_cap_mask[0]);

	ADD_DBG_DATA_TYPE(DBG_DATA_TYPE_ELINK_PHY_INIT);
	ADD_DBG_DATA_SIZE(DBG_DATA_PHY_INIT_SIZE);

	/* Check if link flap can be avoided */
	lfa_status = bnx2x_check_lfa(params);
	ADD_DBG_DATA_BITS(params, LFA_RESULT, lfa_status);
	if (lfa_status == 0) {
				    ...
				    u32 phy_flag, u32 link_flag, u8 notify)
{
	...
	case PHY_SFP_TX_FAULT_FLAG:
		...
		ADD_DBG_DATA_BITS(params, SFP_TX_FAULT, 1);
		break;
	default:
				...
				u8 notify)
{
	...
	if (CHIP_IS_E3(bp) &&
		...
		REG_WR(bp, mac_base + XMAC_REG_CLEAR_RX_LSS_STATUS,
		       XMAC_CLEAR_RX_LSS_STATUS_REG_CLEAR_LOCAL_FAULT_STATUS |
		       XMAC_CLEAR_RX_LSS_STATUS_REG_CLEAR_REMOTE_FAULT_STATUS);
		lss_status = REG_RD(bp, mac_base + XMAC_REG_RX_LSS_STATUS);
		ADD_DBG_DATA_BITS(params, MAC_FAULT, lss_status);
		if (lss_status)
			lss_status = 1;
			       ...
			       struct bnx2x_phy *phy)
{
	...
	if (!sigdet) {
		if (!(params->link_attr_sync & LINK_ATTR_SYNC_KR2_ENABLE)) {
			...
			ADD_DBG_DATA_BITS(params, KR2_WA_NO_SIG, 1);
		}
		return;
	}
	...
	if (base_page == 0) {
		if (!(params->link_attr_sync & LINK_ATTR_SYNC_KR2_ENABLE)) {
			...
			ADD_DBG_DATA_BITS(params, KR2_WA_NO_BP, 1);
		}
		return;
	}

	ADD_DBG_DATA_BITS(params, KR_BP_REG, base_page);
	ADD_DBG_DATA_BITS(params, KR_NP_REG, next_page);
	...
	if (!(params->link_attr_sync & LINK_ATTR_SYNC_KR2_ENABLE)) {
		if (!not_kr2_device) {
			DP(NETIF_MSG_LINK, "BP=0x%x, NP=0x%x\n", base_page,
			   next_page);
			bnx2x_kr2_recovery(params, vars, phy);
			ADD_DBG_DATA_BITS(params, KR2_WA_KR2_DEV, 1);
		}
		return;
	}
	...
	if (not_kr2_device) {
		...
		bnx2x_warpcore_restart_AN_KR(phy, params);
		ADD_DBG_DATA_BITS(params, KR2_WA_KR_DEV, 1);
		return;
	}
}
...
void bnx2x_period_func(struct link_params *params, struct link_vars *vars)
{
	...
	ADD_DBG_DATA_SIZE(DBG_DATA_PERIODIC_SIZE);
	ADD_DBG_DATA_TYPE(DBG_DATA_TYPE_PERIODIC_POLL);
